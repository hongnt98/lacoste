    ...
    if ($body.hasClass('template-product')){
      ...
      function formatMoney(cents, format) {
        var value = '',
            ...
            formatString = (format || window.money_format);
        ...
        for (var a = 0; a < varslength; a++){
          ...
          if(currVar.id == optid){
            if(salePrice <= origPrice){
              $regPriceSpan.css('display', 'none');
              $salePriceSpan.removeClass('fs--xsmall text-grey-light strikethrough').addClass('money fs--medium ff-bold main-price pre-money');
              $salePriceSpan.text(formatMoney(origPrice, window.money)).css('color', '#0d564d');
            } else {
              $salePriceSpan.text('')
              $regPriceSpan.text(formatMoney(salePrice, window.money));
            }
          }
        }
      }

      function reloadUpdateVarPrice(opts){
        var optid = opts.id;
        for (var a = 0; a < varslength; a++){
          var currVar = window.pVars[a],
              salePrice = currVar.price,
          ...
          if(currVar.id == optid){
            if(salePrice <= origPrice){
              $regPriceSpan.text(formatMoney(salePrice, window.money));
              $salePriceSpan.text(formatMoney(origPrice, window.money));
            } else {
              $salePriceSpan.text('')
              $regPriceSpan.text(formatMoney(salePrice, window.money));
            }
...
//                 console.log(triggerPrevColor)
        ...
        $('.js-pdp-color-list a').on('click', function(e){
          e.preventDefault();
          var $this = $(this),
...
//           console.log(color);
      ...
      $('.js-pdp-mobile-size-list .btn-cta').on('click', function(e){
        var $this = $(this),
            $sizeTrigger = $('#trigger-size-panel'),
            $svg = $sizeTrigger.children('svg')[0].outerHTML,
